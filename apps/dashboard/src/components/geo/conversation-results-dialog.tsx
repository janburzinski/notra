...
function EngineResult({ result }: { result: GeoSequenceTurnResult }) {
  ...
  return (
    <span
      className="border-border inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-xs"
      title={`${formatEngineWithMode(result.engine)} · ${result.excerpt}`}
    >
      <EngineIcon className="size-3.5" engine={result.engine} />
      {label}
      {result.mentioned ? (
        <span className="text-geo-up font-medium tabular-nums">
          ...
          {turns.map(([turn, results]) => (
            <div className="flex items-start gap-2" key={turn}>
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div className="border-border bg-muted/40 rounded-2xl rounded-tl-sm border px-3 py-2 text-sm">
                  {results[0]?.prompt}
                </div>
