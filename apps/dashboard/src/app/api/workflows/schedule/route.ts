...
if (contentResult.status === "generation_failed") {
  const autumnClient = autumn;
  if (aiCreditReservation.reserved && autumnClient) {
    await context.run("refund-ai-credit-after-generation-failure", async () => {
      const { error } = await autumnClient.track({
        customer_id: trigger.organizationId,
        feature_id: FEATURES.AI_CREDITS,
        value: 0,
  ...
  await context.run("track-content-failed", async () => {
    try {
      ...
      console.error("[Schedule] Failed to track content generation failure", {
        triggerId,
        organizationId: trigger.organizationId,
        error: trackingError,
      });
...
if (notificationData.enabled && notificationData.ownerEmails.length > 0) {
  await context.run("send-notification-emails", async () => {
    ...
    const baseUrl = process.env.BETTER_AUTH_URL ?? "https://app.usenotra.com";
...
{
  baseUrl: getBaseUrl(), failureFunction;
  : async (context, failStatus, failResponse ) => 
      console.error(
        `[Schedule] Workflow failed
  for trigger ${context.requestPayload.triggerId}
  :`,
    status: failStatus, response
  : failResponse
  )
  ,
}
