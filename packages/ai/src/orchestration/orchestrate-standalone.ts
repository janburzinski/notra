...
const NOTRA_TOOLING_DESCRIPTION =
  "Read-only Notra data tools (GitHub, Linear, Granola, posts, integrations, brand references, skills, web search, webpage fetch, GEO projects, prompt results, and project context) run inside code_mode and are discovered on demand with the searchTools tool before first use. Content, brand identity, GEO chart, and approval tools are called directly. Context.dev tools require API configuration when called.";
